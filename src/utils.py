...
if __name__ == "__main__":
    ...
    class PlayerIntBidirectionalConversionTestCase(unittest.TestCase):
        ...
        def test_player_1(self) -> None: # pylint: disable=no-self-use
            ...
            assert int(Player.P1) == 0, "Player 1 is not 0"
            assert Player(0) == Player.P1, "0 is not Player 1"

        def test_player_2(self) -> None: # pylint: disable=no-self-use
            """Tests that Player 2 is converted to the correct int and vice versa."""
            assert int(Player.P2) == 1, "Player 2 is not 1"
            assert Player(1) == Player.P2, "1 is not Player 2"
    ...
    class SettingsInvalidInputsTestCase(unittest.TestCase):
        ...
        def test_initial_values(self) -> None:
            ...
            assert self.settings.player_name == "Player 1", "Player name is not \"Player 1\""
            assert self.settings.opponent_name == "Player 2", "Opponent name is not \"Player 2\""
            assert self.settings.primary_color == "#573D11", "Primary color is not #573D11"
            assert self.settings.secondary_color == "#DCB167", "Secondary color is not #DCB167"

        def test_invalid_name_change(self) -> None:
            """Tests that invalid names are rejected."""
            self.settings.change_name("This! is! invalid!")
            assert self.settings.player_name == "Player 1", \
        ...
        def test_invalid_hexes(self) -> None:
            ...
            assert not self.settings._is_valid_hex("#invali"), "Invalid hex #invali accepted" # pylint: disable=protected-access
            assert not self.settings._is_valid_hex("#dhexes"), "Invalid hex #dhexes accepted" # pylint: disable=protected-access
            assert not self.settings._is_valid_hex("#12345!"), "Invalid hex #12345! accepted" # pylint: disable=protected-access

        def test_invalid_color_changes(self) -> None:
            """Tests that invalid colors are rejected."""
            self.settings.change_primary_color("#invali")
            assert self.settings.primary_color == "#573D11", \
                f"Primary color was changed from #573D11 to {self.settings.primary_color}"

            self.settings.change_secondary_color("#dhexes")
            assert self.settings.secondary_color == "#DCB167", \
    ...
    class SettingsValidInputsTestCase(unittest.TestCase):
        ...
        def test_valid_name_change(self) -> None:
            ...
            assert self.settings.player_name == "This is valid 123", \
        ...
        def test_valid_hexes(self):
            ...
            assert self.settings._is_valid_hex("#123456"), "Valid hex #123456 not accepted" # pylint: disable=protected-access
            assert self.settings._is_valid_hex("#cf3445"), "Valid hex #cf3445 not accepted" # pylint: disable=protected-access
            assert self.settings._is_valid_hex("#FFF"), "Valid hex #FFF not accepted" # pylint: disable=protected-access

        def test_valid_color_changes(self):
            """Tests that valid colors are accepted."""
            self.settings.change_primary_color("#FFF")
            assert self.settings.primary_color == "#FFF", \
                "Failed to change primary color from #573D11 to #FFF"

            self.settings.change_secondary_color("#cf3445")
            assert self.settings.secondary_color == "#cf3445", \
